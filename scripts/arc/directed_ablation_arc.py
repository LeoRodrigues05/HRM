#!/usr/bin/env python3
"""ARC directed ablation (E9) — project out ARC probe directions, score the damage.

ARC analog of ``scripts/maze/directed_ablation_maze.py`` /
``scripts/controlled/controlled_directed_ablation.py``. Reuses the task-agnostic
projection mechanism (``DirectionalAblator.run_with_directional_ablation``,
z' = z - QQ^T z) but (a) takes its directions from the **ARC** probe weights
produced by ``scripts/arc/arc_linear_probes.py --save_probe_weights``, and (b)
scores the damage on **ARC structure** (colour-cell accuracy / colour IoU /
shape / exact-match), not Sudoku cell accuracy.

For every probe-feature direction (+ N random-direction controls) and every
puzzle it records the Δ in each ARC metric, then reports per direction:
bootstrap 95% CI, paired t + Wilcoxon (probe vs per-puzzle mean random), and
Cohen's d — the same controls used on the Sudoku/Maze side.

Requires: results/arc/hardened/linear_probes/probe_weights.pt
          (run arc_linear_probes.py --save_probe_weights first)

Usage
  python scripts/arc/directed_ablation_arc.py --num_puzzles 500 --z_level H \
      --probe_weights results/arc/hardened/linear_probes/probe_weights.pt \
      --device cuda --output_dir results/arc/hardened/directed_ablation
"""
...
ARC_METRICS = ["colour_cell_acc", "colour_iou", "shape_correct", "exact_solved", "token_acc"]
...
def select_arc_directions(probe_weights: dict, stream: str) -> Dict[str, torch.Tensor]:
...
def main():
    p = argparse.ArgumentParser(description="ARC directed ablation (E9)")
    ...
    p.add_argument("--output_dir", default="results/arc/hardened/directed_ablation")
    args = p.parse_args()
    ...
    if not os.path.exists(args.probe_weights):
        sys.exit(f"ERROR: {args.probe_weights} not found — run "
                 f"scripts/arc/arc_linear_probes.py --save_probe_weights first.")
    pw = torch.load(args.probe_weights, map_location="cpu", weights_only=False)
    directions = select_arc_directions(pw, stream)
    if not directions:
        sys.exit(f"ERROR: no {stream} directions in probe weights.")
    hidden = next(iter(directions.values())).shape[0]
    print(f"[arc_e9] {len(directions)} probe directions ({stream}), hidden={hidden}")
    ...
    print(f"[arc_e9] collected {len(puzzles)} puzzles")
    ...
    for i, (idx, batch) in enumerate(puzzles):
        ...
        for name, direction in all_dirs.items():
            ...
            results_by_dir[name].append({"puzzle_idx": idx,
                **{f"delta_{k}": am[k] - bm[k] for k in ARC_METRICS},
                f"baseline_{PRIMARY}": bm[PRIMARY]})
        if (i + 1) % 50 == 0:
            first = next(iter(directions))
            print(f"  {i+1}/{len(puzzles)} | top probe Δ{PRIMARY} so far = "
                  f"{np.mean([r['delta_'+PRIMARY] for r in results_by_dir[first]]):+.4f}")
    for name in all_dirs:
        print(f"  [{name}] mean Δ{PRIMARY} = "
              f"{np.mean([r['delta_'+PRIMARY] for r in results_by_dir[name]]):+.4f}")
    print(f"[arc_e9] ablations done in {time.time()-t0:.1f}s")
    ...
    for name in directions:
        ...
        rand_pp = [float(np.mean([results_by_dir[rn][pi]["delta_" + PRIMARY] for rn in randoms]))
                   for pi in range(n)]
        try:
        ...
        analysis[name] = {
            f"probe_delta_{PRIMARY}": bootstrap_ci(probe),
            f"random_control_delta_{PRIMARY}": bootstrap_ci(rand_pp),
            "probe_delta_exact_solved": bootstrap_ci([r["delta_exact_solved"] for r in results_by_dir[name]]),
            "probe_delta_colour_iou": bootstrap_ci([r["delta_colour_iou"] for r in results_by_dir[name]]),
    ...
    try:
        ...
        write_meta(args.output_dir, "arc_directed_ablation", {
        ...
        print(f"[arc_e9] WARN _meta: {e}")

    print("\n" + "=" * 80)
    print(f"{'direction':26s} {'probe Δ':>12s} {'random Δ':>12s} {'Cohen d':>8s} {'p':>10s}")
    for name, a in analysis.items():
        print(f"{name:26s} {a['probe_delta_'+PRIMARY]['mean']:>+12.4f} "
              f"{a['random_control_delta_'+PRIMARY]['mean']:>+12.4f} "
              f"{a['cohens_d']:>8.3f} {a['paired_p_value']:>10.5f}")
    print(f"[arc_e9] wrote {args.output_dir}")
