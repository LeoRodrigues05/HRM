...
# ── 0. dataset ────────────────────────────────────────────────────────────
if [ "${FORCE_REBUILD:-0}" = "1" ] || [ ! -f data/arc-2-aug-1000/test/dataset.json ]; then
...
run "E9 directed ablation z_L" "$PY" -u scripts/arc/directed_ablation_arc.py \
    ...
    --device "$DEVICE" --output_dir "${DA}_zL"
